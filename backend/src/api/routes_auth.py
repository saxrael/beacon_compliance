...
async def login_with_password(
    ...
    input_pwd_hash = hmac.new(
        salt.encode(), f"{req .password }:{salt }".encode(), hashlib.sha256
    ).hexdigest()
...
async def login_with_2fa(
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="Invalid temporary token.") from err
...
class Generate2FAAuthResponse(BaseModel):
    secret: str
    provisioning_uri: str


class Enable2FAAuthRequest(BaseModel):
    totp_code: str
...
@router.post("/2fa/generate", response_model=Generate2FAAuthResponse)
async def generate_2fa_auth(
    current_user: TrusteeUser = Depends(get_current_trustee),
    db: D1DatabaseClient = Depends(get_d1_db),
) -> Generate2FAAuthResponse:
    """Generate a new TOTP secret for the authenticated user (alias)."""
    user = db.fetchone("SELECT * FROM users WHERE user_id = ?", (current_user.user_id,))
    if not user:
    ...
    return Generate2FAAuthResponse(secret=secret, provisioning_uri=provisioning_uri)


@router.post("/2fa/enable")
async def enable_2fa_auth(
    req: Enable2FAAuthRequest,
    current_user: TrusteeUser = Depends(get_current_trustee),
    db: D1DatabaseClient = Depends(get_d1_db),
) -> dict[str, str]:
    """Verify the first TOTP code and enable 2FA (alias)."""
    user = db.fetchone("SELECT * FROM users WHERE user_id = ?", (current_user.user_id,))
...
async def change_password_auth(
    ...
    """Change user password, enforcing 2FA if enabled (alias)."""
    user = db.fetchone("SELECT * FROM users WHERE user_id = ?", (current_user.user_id,))
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")
