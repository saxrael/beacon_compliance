"""Strip non-whitelisted comments from repository files.



Rules:

- Preserve comments that contain the token "ruff" (case-insensitive) or "prompt" (case-insensitive).

- Preserve shebang lines (#!) and Python docstrings.
...
- Preserve comments that contain the token "ruff" (case-insensitive) or "prompt" (case-insensitive).
- Preserve shebang lines (#!) and Python docstrings.
- Operate on file extensions: .py, .js, .ts, .jsx, .tsx, .sh, .env, .yaml, .yml
- Backup original files under .comment_backups/<relative_path>.orig when applying changes.

Usage: python scripts/remove_comments.py [--apply]
By default the script runs in dry-run mode and does not write files. Use `--apply` to make changes.
"""
...
def process_python_file(path: Path, dry_run: bool) -> int:
    """Remove COMMENT tokens except whitelisted; preserve docstrings and shebang."""
    changed = 0
...
def process_js_like_file(path: Path, dry_run: bool = True) -> int:
    """Remove // and /* */ comments conservatively, preserving whitelist tokens."""
    changed = 0
