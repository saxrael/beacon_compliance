...
def provision_trustee(email: str, name: str, role: str, db_path: str | None = None) -> dict[str, str]:
    ...
    if normalized_role not in ("Chair", "Secretary", "Treasurer", "Trustee", "Admin", "Developer"):
        raise ValueError(f"Invalid role '{role}'. Must be Chair, Secretary, Treasurer, Trustee, Admin, or Developer.")
...
def main():
    ...
    parser.add_argument("--role", required=True, choices=["Chair", "Secretary", "Treasurer", "Trustee", "Admin", "Developer"], help="Trustee executive or admin role")
